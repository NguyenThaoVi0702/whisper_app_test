...
QUANTIZATION = "float16"  # Best for quality on A100; use "int8" only if VRAM tight
...
# Copy all config/tokenizer files (excludes large weights)
print(f"Copying config and tokenizer files from '{SOURCE_MODEL_PATH}'...")
all_files = os.listdir(SOURCE_MODEL_PATH)
files_to_exclude = {"pytorch_model.bin", "model.safetensors", "adapter_model.safetensors"}  # Post-merge exclusions
...
# After conversion, CT2 generates 'config.json' and 'model.bin'. If alignment_heads error persists,
# edit the new config.json in CT2 dir with this offline code (run post-conversion):
...
if os.path.exists(ct2_config_path):
    ...
    # Ensure alignment_heads is present (CT2 may derive it, but manual add if error; turbo-specific)
    ...
    # Add suppress_ids and lang_ids from coworker's CT2 for turbo compatibility
